...
export const Disbursements = () => {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "QUEUED":
        return <Badge className="bg-warning text-warning-foreground"><AlertCircle className="w-3 h-3 mr-1" />Queued</Badge>;
      case "FAILED":
  ...
  const filteredDisbursements = user.role === "PRODUCER" 
    ? mockDisbursements.filter(d => d.producer === "HydroCorp Ltd") // Simulate user's disbursements
    : mockDisbursements;
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold">
              {user.role === "PRODUCER" ? "My Payments" : 
               user.role === "BANK" ? "Settlement Queue" : "Disbursements"}
            </h1>
            <p className="text-muted-foreground">
              {user.role === "PRODUCER" ? "Track your subsidy payments and wallet credits" :
               user.role === "BANK" ? "Process and manage subsidy disbursements" :
              ...
              <CardHeader>
                <div className="flex items-start justify-between">
                  <div className="space-y-1">
                    <CardTitle className="text-lg">{disbursement.schemeName}</CardTitle>
                    <p className="text-sm text-muted-foreground">
                      {user.role !== "PRODUCER" && `Producer: ${disbursement.producer} • `}
                      ID: {disbursement.id}
                    </p>
                  </div>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">{disbursement.amountEth}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="font-mono text-sm">{disbursement.contractAddress}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                  ...
                  {disbursement.processedDate && (
                    <div className="text-sm text-muted-foreground">
                      Processed on {new Date(disbursement.processedDate).toLocaleDateString()}
                    </div>
                  )}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {user.role === "BANK" && disbursement.status === "PENDING" && (
                      <Button size="sm" className="bg-gradient-primary text-primary-foreground hover:opacity-90">
                        Process Payment
                      </Button>
                    )}
                    {user.role === "GOV" && disbursement.status === "QUEUED" && (
                      <Button size="sm" className="bg-gradient-primary text-primary-foreground hover:opacity-90">
                    ...
                    <div className="p-3 bg-success/10 border border-success/20 rounded-lg">
                      <div className="flex items-center gap-2 text-success">
                        ...
                        <span className="text-sm font-medium">
                          {user.role === "PRODUCER" ? "Wallet Credited Successfully" : "Payment Completed"}
                        </span>
                      </div>
                      {disbursement.txHash && (
