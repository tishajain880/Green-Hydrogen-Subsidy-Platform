export { default } from "./Milestones.jsx";
export * from "./Milestones.jsx";
...
export const Milestones = () => {
  ...
  useEffect(() => {
    // Use the API client directly, load submissions and projects,
    // subscribe to project SSE and fall back to polling.
    let mounted = true;
    ...
    const loadProjects = async () => {
      try {
        // Choose which projects to load depending on role:
        // - PRODUCER: show approved/public projects to enroll in
        // - GOV / admin: show all projects
        // - others: show projects owned by user
        const session = localStorage.getItem("userSession");
    ...
    try {
      ...
      es.addEventListener("project", () => {
        // refresh projects on any project event
        loadProjects();
      });
      es.addEventListener("error", () => {
        // SSE errored — we'll keep polling as fallback
      });
    } catch (e) {
      es = null;
    }
  ...
  // Use real user.milestones from DB if present; otherwise assume empty
              ...
              <form
                onSubmit={async (e) => {
                  ...
                  try {
                    ...
                    const files = (
                      document.getElementById("proofFiles") as HTMLInputElement
                    ).files;
                    if (files) {
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {user.role === "admin" &&
                        milestone.status === "PENDING" && (
                          <>
                            {/* Find a pending submission matching this milestone (by milestoneIndex) */}
                            {(() => {
                              const mIndex = Number(
                                milestone.index ??
